...
class RabbitMqClient {
    ...
    private __produceChannel: Channel | undefined;
    ...
    async initialize() {
        ...
        try {
            ...
            this._connection = await connect(rabbitmqConfig.rabbitMQ.url);
            const [produceChannel, consumerChannel] = await Promise.all([
                this._connection.createChannel(),
                this._connection.createChannel()
            ]);

            this.__produceChannel = produceChannel;
            ...
            const { queue: replyQueueName } = await this._consumerChannel.assertQueue("", { durable: true });
            this._eventEmitter = new EventEmitter();

            this._producer = new Producer(this.__produceChannel, replyQueueName, this._eventEmitter);
